    ...
    <body>
        ...
        <script>
                                            ...
                                            window.addEventListener('load', function () {
                                                ...
                                                // Only set default dates if they're not already populated
                                                ...
                                                // Populate dates from selected registration if available
            <c:if test="${not empty selectedRegistration}">
                <c:if test="${not empty selectedRegistration.startDate}">
                                                document.getElementById('startDate').value = "${selectedRegistration.startDate}".split(' ')[0];
                </c:if>
                <c:if test="${not empty selectedRegistration.endDate}">
                                                document.getElementById('endDate').value = "${selectedRegistration.endDate}".split(' ')[0];
                </c:if>
            </c:if>
                                            });

                                            // Load shift registration dates when registration is selected
                                            document.getElementById('shiftRegistrationId').addEventListener('change', function () {
                                                const registrationId = this.value;
                                                if (!registrationId)
                                                    return;

                                                // AJAX call to get registration data
                                                $.ajax({
                                                    url: 'schedule-management',
                                                    type: 'GET',
                                                    data: {
                                                        action: 'getRegistrationDates',
