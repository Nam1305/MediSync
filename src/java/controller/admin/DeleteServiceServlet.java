...
public class DeleteServiceServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean isDelete = serviceDao.deleteService(serviceId);
            if (serviceDao.getServiceById(serviceId) == null) {
                request.setAttribute("error", "Not found service. Please try again.");
                request.getRequestDispatcher("ListService").forward(request, response);
            }
            if (isDelete) {
                response.sendRedirect("ListService");
            } else {
                request.setAttribute("error", "Failed to delete Service. Please try again.");
                request.getRequestDispatcher("ListService").forward(request, response);
            }
